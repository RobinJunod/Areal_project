...
class MyController():
    def __init__(self):
        ...
        self.height_desired = flying_height

        self.x_drone, self.y_drone = 0,0
        self.liftoff_point_x, self.liftoff_point_y = 0,0
        self.goal_x, self.goal_y = 4.95,0.3 #Initial goal set to middle of end zone
    ...
    def step_control(self, sensor_data):
        ...
        self.x_drone, self.y_drone = sensor_data['x_global'], sensor_data['y_global']
        ...
        if self.state == 'CROSSING_MAP':
            ...
            control_command = track_setpoint(self.goal_x, self.goal_y, self.x_drone, self.y_drone, sensor_data, self.height_desired, crossing_speed)
        ...
        # SEARCHING_LANDING_PAD #
            ...
            control_command = track_setpoint(x_target, y_target, self.x_drone, self.y_drone, sensor_data, self.height_desired, exploration_speed)
        ...
        # LANDING_PAD_CENTERING STATE #
            ...
            if not self.stopped_on_pad_flag :
                if (abs(sensor_data['v_forward']) > speed_stable_threshold or abs(sensor_data['v_left']) > speed_stable_threshold or abs(sensor_data['yaw']) > 0.05) :
                    # Orient toward 0 yaw
                    yaw_goal = 0
                    yaw_rate = yawing_P*(((yaw_goal - sensor_data['yaw'])+np.pi)%(2*np.pi)-np.pi) + yawing_D * sensor_data['yaw_rate']
            ...
            # Find X edge
            elif not self.landing_pad_x_pos_edge_found :
                control_command = [landing_pad_scanning_speed, 0.0, 0.0, self.height_desired]
                
                # If vertical edge detected -> store x pos
                if abs(sensor_data['range_down'] - self.previous_range_down) > edge_detection_threshold :
                    self.landing_pad_x_pos_edge_found = True
                    self.landing_pad_x_pos_edge = sensor_data['x_global']

            # Find Y edge if X centered
            elif self.landing_pad_x_pos_edge_found and (not self.landing_pad_y_pos_edge_found) and abs(((self.landing_pad_x_pos_edge - landing_pad_size/2) - self.x_drone)) < 0.02:
                control_command = [0.0, landing_pad_scanning_speed, 0.0, self.height_desired]
            ...
            # Center Y and X if pad center found
                ...
                control_command = track_setpoint(self.goal_x, self.goal_y, self.x_drone, self.y_drone, sensor_data, self.height_desired, centering_speed)
            ...
            # X centering if x edge found
            if self.landing_pad_x_pos_edge_found  and not self.landing_pad_y_pos_edge_found:
                v_front = P_vel * ((self.landing_pad_x_pos_edge - landing_pad_size/2) - self.x_drone)
                v_front = np.clip(v_front, -landing_pad_scanning_speed, landing_pad_scanning_speed)
                control_command[0] = v_front
        ...
        # RETURN_HOME STATE #
            ...
            if self.height_desired < flying_height :
                ...
                control_command = track_setpoint(self.goal_x, self.goal_y, self.x_drone, self.y_drone, sensor_data, self.height_desired, return_speed)
        ...
        # TAKEOFF STATE #
            ...
            if (sensor_data['range_down'] < 0.01) :
                self.liftoff_point_x, self.liftoff_point_y = sensor_data['x_global'], sensor_data['y_global']
...
def track_setpoint(setpoint_x, setpoint_y, x_drone, y_drone, sensor_data, height_desired, nominal_speed) :
    ...
    new_vel_x_target = nominal_speed * (force_x * np.cos(sensor_data['yaw']) + force_y * np.sin(sensor_data['yaw']))
    new_vel_y_target = nominal_speed * (- force_x * np.sin(sensor_data['yaw']) + force_y * np.cos(sensor_data['yaw']))
